...
function formatCounters(counters, prefix = '') {
  const strings = Object.entries(counters).map(([n, v]) => `${prefix}${v.toString().padStart(NUMBER_LEN)} ${n}`);
  return strings.join('\n');
}

function formatTiming(counter, name, nanos, prefix = '') {
  const nanosStr = toHuman(nanos);
  const counterStr = counter.toString().padStart(NUMBER_LEN);
  const nameStr = name.padEnd(NAME_LEN);
